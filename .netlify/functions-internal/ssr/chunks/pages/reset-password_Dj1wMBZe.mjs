...
import { R as ResetPassword, U as UserModel } from './forgot-password_D88SrvWL.mjs';
...
async function POST(context) {
  ...
  const tokenHash = encodeHex(await sha256(new TextEncoder().encode(verificationToken.toString())));
  ...
  await UserModel.findByIdAndUpdate(token.user_id, { password: hashedPassword });
